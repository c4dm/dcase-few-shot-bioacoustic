...
def compute_scores_per_class_and_average_scores_per_set(counts_per_class):
    ...
    #harmonic mean with zero entries gives zeros :( )
    ...
    av_scores_set = {"precision": sum(cumulative_precision)/n_classes, "recall": sum(cumulative_recall)/n_classes, "f-measure": sum(cumulative_fmeasure)/n_classes }
    
    # # instead of average per class average per audiofile for consistency when we do not have the metadata
    # av_scores_set = {"precision": sum(cumulative_precision)/n_classes, "recall": sum(cumulative_recall)/n_classes, "f-measure": sum(cumulative_fmeasure)/n_classes }
...
def evaluate( pred_file_path, ref_file_path, team_name, dataset, savepath, metadata=[]):
    ...
    for subset in list_of_subsets:
        # gt_file_structure[dataset][subset] = os.listdir(os.path.join(ref_file_path,subset)) # change to read only .csvs, but save it as .wav?
    ...
    # if the scores for a subset are 0, harmonic mean gives zero( div by zero should give error!)
